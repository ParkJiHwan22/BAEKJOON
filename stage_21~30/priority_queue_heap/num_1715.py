...
# 입력
# 첫째 줄에 N이 주어진다. (1 ≤ N ≤ 100,000) 이어서 N개의 줄에 걸쳐 숫자 카드 묶음의 각각의 크기가 주어진다. 숫자 카드 묶음의 크기는 1,000보다 작거나 
# 같은 양의 정수이다.
...
heapq.heapify(card) # 이걸 꼭 써야 하는 이유? 힙을 만들려면, []로 초기화된 리스트를 사용하거나, 
# 함수 heapify()를 통해 값이 들어 있는 리스트를 힙으로 변환 할 수 있음
min = tmp_1 = tmp_2 = 0
